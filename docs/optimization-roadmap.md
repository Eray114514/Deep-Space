...
迁移已完成约 90%,WebGPU 路径能启动渲染、不黑屏,`npm test` 全过,`test:renderers` 中 `fixed-vista parity gate` 通过。以下是已知的遗留问题,但**本路线图不是去修复这些迁移遗留,而是通过全面重构顺便解决它们**:

| 遗留问题 | 根因 | 由哪个重构批次解决 |
|---|---|---|
| 大气深度裁剪未接入 | TSL 大气材质声明了 `tSceneDepth`/`uDepthReady` 但未使用 | 批次 B:大气与云深度重写 |
| 体积光散射(god-ray)缺失 | WebGPU 后处理链无径向模糊 pass | 批次 B:大气与云深度重写 |
| 后处理顺序与 WebGL 不同 | warp/rift 在 volume 之前而非之后 | 批次 B:重构后处理链 |
...
| ~3300m 掉帧至 1fps(记录,不修复) | 推测是 farflora 预热构建/LOD 切换/材质编译的瞬时开销 | 批次 A/C 顺带观察是否消失 |
...
- **太空看海洋远看效果假(新增痛点)**:WebGPU 路径下太空看海洋是一片完全一样的劣质蓝色,远看无波纹/深度变化/ Fresnel 随距离衰减。推测根因是海面 LOD 与地形 LOD 共享 quadtree 但海面材质在远距离无细节 contribution,且 WebGPU 的 `applyWaterWaves` TSL 实现在低 LOD 下退化为纯色。
...
- [ ] **太空看海洋假**:对比不同 LOD level 下海面材质输出,确认是否是远距离无细节 contribution;测试海面是否需要独立 LOD 策略(非 quadtree 而是球面 impostor)
...
- 海面 LOD:海面和地形共享 quadtree LOD 但海面材质需要远距离 fallback(球面 impostor + 距离相关 Fresnel),不能在低 LOD 下退化为纯色
...
| 日期 | 决策 | 理由 |
|---|---|---|
| 2026-07-26 | 植物系统全部禁用并完全重做(非优化) | TSL `positionLocal` 无 instanceMatrix 是架构性限制,修补无效 |

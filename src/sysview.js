...
export class SystemView {
  constructor({ host, labelHost, navArrow, nameTag, onSelect, pixelRatioCap = 1.75 }) {
    this.host = host;
    ...
    // WebGLRenderer is synchronously ready; expose the same ready/await API
    // starmap.js relies on for switching into the system preview.
  ...
  // WebGL has no pending GPU work to fence, so we just release the retired
  // scene objects immediately and resolve. The method keeps a Promise shape
  // so starmap.js can `await` it the same way the old WebGPU path expected.
  suspend() {

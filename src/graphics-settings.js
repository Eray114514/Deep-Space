...
export function chooseAutomaticQuality(gpuName = '', { touch = false, width = 1920, height = 1080 } = {}) {
  if (isLowPowerGpu(gpuName) || touch) return 'performance';
  if (width * height > 2560 * 1440) return 'balanced';
  return /RTX\s*(?:40[789]0|50\d0)|RX\s*(?:79|89)/i.test(gpuName) ? 'ultra' : 'balanced';
}

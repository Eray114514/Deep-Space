...
try {
  for (const scene of ['planetary', 'black-hole', 'switch']) {
    ...
    const ok = result.backend === 'webgl2' && result.nodeMaterials > 0
  ...
  if (webgpuBrowser) {
    try {
      for (const scene of ['black-hole', 'switch']) {
        ...
        await page.goto(`http://127.0.0.1:${port}/tools/sysview-fixture.html?renderer=webgpu&scene=${scene}`);
        await page.waitForFunction(() => window.SYSVIEW_RESULT?.ready, null, { timeout: 90000 });
        const result = await page.evaluate(() => window.SYSVIEW_RESULT);
        const ok = result.backend === 'webgpu' && result.nodeMaterials > 0
          && result.previewId === result.expectedPreviewId && result.bodyCount > 0 && errors.length === 0;
        console.log(`${ok ? 'PASS' : 'FAIL'}: sysview ${scene} ${result.backend}, ${result.nodeMaterials} node materials`);
        if (errors.length) console.log(errors.join('\n'));
        if (!ok) failures++;

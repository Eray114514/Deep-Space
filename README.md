...
- **过程化宇宙**：无限星系晶格，每个单元格哈希出（可能存在的）恒星；每颗恒星的系统——太阳颜色、行星数量、类型、轨道、卫星、命名——全部由坐标确定性生成。天空中你能看到的每一颗星都是真实可达的目标（约 15,000 颗在视），无虚假 skybox。
...
- **黑洞观测站**：厄瑞玻斯引力弹弓观测，基于 Eric Bruneton 相对论光线模型，配独立版本化 RNG 命名空间，不重摇任何既有星系。
...
发布构建始终使用策展过的 `NAVEMI-382` 种子与 `milky-way`（银河系）。种子覆写是仅开发期的世界实验室工具：`npm run dev` 后用 `?worldlab=1&seed=ANYTHING`；其他开发参数含 `&nolock=1`（无指针锁定）、`&galaxy=milky-way`、`&system=0,0,0&body=planet-0&sea=-420&clouds=0.55`。永久策展内容应落在 `src/world-config.js`，过程化生成器保持种子纯净。
...
发布宇宙是策展过的 `milky-way` 星系，种子 `NAVEMI-382`。`src/world-config.js` 是其身份、特殊目的地与稳定 `galaxy ID / system ID / body ID` 调参的成文来源。过程化生成器仍是无限星系的运行时来源，不要用有限烘焙宇宙替换它们，也不要把存档当作世界内容。

`worlds/milky-way.lock.json` 是人类可读的兼容性快照，记录完整的母系档案、成文黑洞目的地、18 个最近星系、64 系统邻域剖面、母星地形/海/云/环哨兵、生成器版本与 SHA-256 指纹。它**不**序列化整个无限晶格、渲染对象、纹理或玩家状态。仅在有意重新策展后用 `npm run world:lock` 重建，绝不为让失败测试通过而重建。
...
- **配乐**（`assets/audio/` 下 18 首 MP3）：由 **Suno AI** 生成，按**非商业**用途随本项目分发，**不**随 MIT 许可释放。曲目目录与场景绑定见 `src/music.js`。如需商业使用，须另行获取相应授权。
- **黑洞引力弹弓观测站**（`assets/vendor/black-hole/`）：改编自 MakerJackie 的同名作品（MIT），其底层实时相对论渲染器源自 Eric Bruneton 的 Black Hole Shader（BSD 3-Clause），恒星数据来自 Gaia DR2 与 Tycho-2。完整许可与声明见 `THIRD_PARTY_NOTICES.md` 与该目录下的 `MAKERJACKIE-LICENSE.txt`。
